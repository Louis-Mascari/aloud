...
Replace `VOICE=~/Desktop/personal/claude-voice/bin/voice` with your path.
...
```
cmd + alt - period : ~/Desktop/personal/claude-voice/bin/voice stop
```
Already run Hammerspoon? `~/.hammerspoon/init.lua`:
```lua
hs.hotkey.bind({"cmd","alt"}, ".", function() hs.execute("~/Desktop/personal/claude-voice/bin/voice stop") end)
```

**Linux — bind in your compositor/DE (Wayland-safe, no root):**
- sway/Hyprland: `bindsym Ctrl+Shift+s exec $VOICE stop`
...
**tmux** (`~/.tmux.conf`):
```
bind-key -n F13 run-shell "~/Desktop/personal/claude-voice/bin/voice stop &"
```
**kitty** (`kitty.conf`):
```
map f13 launch --type=background ~/Desktop/personal/claude-voice/bin/voice stop
```
