...
reading walls of text. Local, work-safe, and quiet when you need it to be. Part
of the [aloud](../README.md) collection.
...
```bash
git clone <this repo> ~/aloud
~/aloud/install.sh      # backs up, then wires hooks + CLAUDE.md
```

Then, if you use WezTerm, apply [`../wezterm/INTEGRATE.md`](../wezterm/INTEGRATE.md) to your `wezterm.lua` (`~/.wezterm.lua` or `~/.config/wezterm/wezterm.lua`)
...
```bash
echo 'export PATH="$HOME/aloud/bin:$PATH"' >> ~/.zshrc   # or ~/.bashrc
```
...
The shared Kokoro engine (`kokoro/`, `bin/kokoro-say`, `setup-kokoro.sh`) is used
by every tool. See [`architecture.md`](architecture.md) for the full event/state map.
